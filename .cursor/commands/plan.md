...
- **Planning is read-only**: during the planning session, the agent MUST NOT CHANGE any code or repository state (no patches, no edits, no file moves/renames, no generated files, no formatting runs, no dependency installs). Only read files, inspect logs, and ask questions; wait for explicit user approval and a separate implementation request before making changes.
...
- **No backward-compat constraint**: do not spend effort preserving backward compatibility (APIs, file formats, CLI flags) unless the user explicitly asks.
