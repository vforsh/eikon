...
export interface AdjustBrightnessOptions {
  out: string;
  factor?: string;
  force?: boolean;
  json?: boolean;
...
export async function adjustBrightnessCommand(image: string, opts: AdjustBrightnessOptions) {
  if (!image) {
    throw new UsageError("Missing <image> argument", [
      "Usage: eikon adjust brightness <image> --out brightened.png",
    ]);
  }
  ...
  let pipeline = sharp(inputBuffer).modulate({ brightness: factor });
