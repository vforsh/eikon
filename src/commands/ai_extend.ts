...
export interface AiExtendOptions {
  direction?: string;
  pixels?: string;
  prompt?: string;
  out: string;
...
const VALID_DIRECTIONS = ["up", "down", "left", "right", "all"];

const MOCK_PNG_BASE64 =
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/xcAAn8B9nL8rW0AAAAASUVORK5CYII=";

export async function aiExtendCommand(image: string, opts: AiExtendOptions) {
  ...
  if (!VALID_DIRECTIONS.includes(direction)) {
    throw new UsageError(`Invalid --direction: ${direction}`, [
      `Valid directions: ${VALID_DIRECTIONS.join(", ")}`,
    ]);
  }

  // Parse pixels
  const pixelsStr = opts.pixels || "256";
  const pixels = parseInt(pixelsStr, 10);
  if (!Number.isFinite(pixels) || pixels <= 0) {
    throw new UsageError(`Invalid --pixels: ${pixelsStr}`, [
  ...
  const directionText = direction === "all" ? "in all directions" : direction;
  const instruction = await loadAiPromptWithVars("extend", {
