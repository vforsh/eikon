...
export interface FxShadowOptions {
  out: string;
  color?: string;
  opacity?: string;
  blur?: string;
...
export async function fxShadowCommand(image: string, opts: FxShadowOptions) {
  if (!image) {
    throw new UsageError("Missing <image> argument", [
      'Usage: eikon fx shadow <image> --out shadow.png',
    ]);
  }
  ...
  // Calculate canvas expansion needed for shadow
  // Shadow extends by: blur radius + spread + offset
  const blurExpand = Math.ceil(blurRadius * 3); // 3-sigma covers ~99.7%
  const totalExpand = blurExpand + spread;

  const padTop = Math.max(0, totalExpand - dy);
  const padBottom = Math.max(0, totalExpand + dy);
  const padLeft = Math.max(0, totalExpand - dx);
  const padRight = Math.max(0, totalExpand + dx);

  const canvasWidth = originalInfo.width + padLeft + padRight;
  const canvasHeight = originalInfo.height + padTop + padBottom;
  ...
  // Step 3-5: Blur alpha if needed, then scale by opacity
  ...
  // Step 6: Assemble RGBA shadow layer (solid color + processed alpha)
  const shadowRgba = Buffer.alloc(srcW * srcH * 4);
  ...
  // Step 7: Create transparent canvas and composite shadow + original
  // Shadow position on canvas: offset from where the original sits, shifted by dx/dy
  const origLeft = padLeft;
  const origTop = padTop;
  const shadowLeft = origLeft + dx;
  const shadowTop = origTop + dy;
