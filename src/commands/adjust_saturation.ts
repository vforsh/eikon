...
export interface AdjustSaturationOptions {
  out: string;
  factor?: string;
  force?: boolean;
  json?: boolean;
...
export async function adjustSaturationCommand(image: string, opts: AdjustSaturationOptions) {
  if (!image) {
    throw new UsageError("Missing <image> argument", [
      "Usage: eikon adjust saturation <image> --out saturated.png",
    ]);
  }
  ...
  let pipeline = sharp(inputBuffer).modulate({ saturation: factor });
